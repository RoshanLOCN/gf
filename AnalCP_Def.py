...
class BlockingProbability:
    ...
    def firstFitExec(self, count_out, count_inc, transitionIncState):
        # count_out, count_inc = self.firstFit()
        # # print(len(self.states))
        # # for i in range(len(self.states)):
        # #     print(i)
        # #     self.mprint(self.states[i])
    ...
    def __init_resource_matrix(self):
        return [[0 for i in range(self.slot)] for j in range(self.col)]
    ...
    def interferncefillAdj(self, row, col, size, flip):
        ...
        # coreCol = self.isLastCoreCol(col)
        try:
                # ab=self.int_list[col]
            # if self.int_list[col]!=0:
                # for j in range(1, self.core):
                    # if col + (j * self.mode) >= self.col:
                    #     break
                    # if self.isAdj(col, col + (j * self.mode)):
                        # print(col, j+self.mode)5
                        # flag = False
                if  self.int_list[col]%2!=0 and self.int_list[col]!=0:
                        for i in range(size):
                            if self.matrix[row + i][col]==1:
                                self.matrix[row +i][col + 1] = -1
                if self.int_list[col] % 2 == 0 and self.int_list[col]!=0:
                         for i in range(size):
                            if self.matrix[row - i][col] == 1:
        ...
        while currInd <= len(self.states) - 1:
            for j in range(len(self.states[currInd])):
                for x in self.classes:
                    valueChecked = [0] * x
                    state_check = 0
                    while state_check <= len(self.states[currInd][j]) - x:
                        if self.states[currInd][j][
                           state_check:  state_check + x] == valueChecked and state_check + x < self.slot and \
                                self.states[currInd][j][state_check + x] == 0:
                            tempMatrix = copy.deepcopy(self.states[currInd])
                            tempMatrix[j][state_check:  state_check + x] = [1] * x
                            self.matrix = tempMatrix
                            self.matrix = np.transpose(self.matrix)
                            self.interferncefillAdj(state_check, j, x, False)
                            # self.RandomFitfillAdj(state_check, j, x, False)
                            self.matrix = np.flip(self.matrix)
                            self.interferncefillAdj(state_check, j, x, True)
                            # self.RandomFitfillAdj(self.row-state_check-1, self.col-j-1, x,True)
                            self.matrix = np.flip(self.matrix)
                            self.matrix = np.transpose(self.matrix)
                            tempMatrix = self.matrix.tolist()
                            ...
                            state_check += 1
                        if self.states[currInd][j][
                           state_check:  state_check + x] == valueChecked and state_check + x == self.slot:
                            # if (state_check+x)%x !=0:
                            #     continue
                            tempMatrix = copy.deepcopy(self.states[currInd])
                            tempMatrix[j][state_check:  state_check + x] = [1] * x
                            self.matrix = tempMatrix
                    ...
                    for state_check in range(len(self.states[currInd][j])):
                        ...
                        if self.states[currInd][j][
                           state_check:  state_check + x] == valueChecked and state_check + x == self.slot:
                            ...
                            tempMatrix = copy.deepcopy(self.states[currInd])
                            tempMatrix[j][state_check:  state_check + x] = [1] * x
                            # tempMatrix[j][state_check + x]=-1
                            self.matrix = tempMatrix
                            self.matrix = np.transpose(self.matrix)
                            # self.self.createFirstFitInitialStates()
                            #         slotArr = []
                            #         transitionArr = []
                            #         for currInd in range(len(self.states)):
                            #             slotValue = {}
                            ...
                            #             slotArr.append(slotValue)
                            #             transitionArr.append(finalClassesTransition)
                            #         return slotArr, transitionArr(state_check, j, x , False)
                            self.interferncefillAdj(state_check, j, x, False)
                            self.matrix = np.flip(self.matrix)
                            self.interferncefillAdj(state_check, j, x, False)
                            # self.RandomFitfillAdj(self.row-state_check-1, self.col-j-1, x,True)
    ...
    def calcBP(self, output, bp):
        ...
        s = [0 for i in range(len(self.classes))]
        cs = [0 for i in range(len(self.classes))]
        ...
        # print(cs)
        print(s)
        # print(self.lambda_val, sum(self.lambda_val))
        N = 0
        for i in range(len(s)):
            # print(N)
            N+=(s[i]*self.lambda_val[i])
        D = sum(self.lambda_val)
        # print(N,D)
        N2 = 0
        for i in range(len(s)):
            N2+=(((1-s[i])*self.lambda_val[i]*self.classes[i])/self.mu_val[i])
        b_p = N/D
        sigma = N2
